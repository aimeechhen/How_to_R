...
Before analysis, we calibrated the GPS measurement error and removed any outlying data points (for full details on data pre-processing see Appendix S2 in (Noonan et al., 2022). 

## Data Import

First step of the analysis is to import the data and convert to a telemetry object for use with the ```ctmm``` package. 
...
We restricted our analyses to 23 range-resident individuals living in three separate clusters. These individuals were selected as they resided in areas where there was high confidence that all resident giant anteaters were equipped with GPS trackers. 
...
#Visualisation of the telemetry data
plot(DATA_TELEMETRY)
```
...
Home range size of each individual
...
#add biological information to dataframe
...
Home range size based on a sex parameter using meta() from the ctmm package
Meta-analysis for home range size statistics based on sex via ctmm package??
...
Home range overlap based on each study site location
...
Create a pairwise dataframe by pairing up every individual for each site
...
Home range overlap with sex as a variable
...
Home range overlap results based on sex categories (ie. male-male, female-female, and female-male)
...
Proximity ratio with sex as a variable (male, female)
...
We also estimated the Euclidean distance between the individuals in each dyads at each timepoint using the ```ctmm``` function ```distances()```. From these separation distances we estimated the number of encounter events for each dyad using a 15m distance threshold. We performed a sensitivity analysis on the 15m threshold.

### Estimating distance

Distance measurement metrics at each time interval between each individual
...
Determine the distance threshold to be used as an encounter event???
...
fig3c_2
ggsave(fig3c_2, width = 6.86, height = 3, units = "in", dpi = 600, bg = "transparent",
       file="figures/fig3c_2.png")
...
### Identified Proximity Pairs Analysis
...
### Estimating Distances of the Identified Proximity Ratio Pairs 
...
### Correlative Movement Results/Analysis??
...
s#mean 95% CI correlative movement
